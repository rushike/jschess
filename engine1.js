...
const x88_MOVE_BITS = 11

const x88_MOVE_MASK = 0x7FF
...
const __PIECE_VALUE = [1023, 127, 64, 47, 47, 23, 0, 0, -1023, -127, -64, -47, -47, -23, 0, 0];
...
class Board{
    ...
    valid_moves(j, i){ // i : x  ; j : y
        ...
        var moves = this.engine.valid_moves(Square.x88sqno(i, j), null, 0, true)
        this.val_moves = moves
        this.v_moves = this.to_visual_array(moves)
        console.log("val moves : ", this.val_moves)
        this.v_moves.forEach(c =>{
    ...
    move(n_j, n_i){
        this.engine.move(Square.x88sqno(n_i, n_j), null, this.val_moves)
        this.update()   
    }
...
class PRE_ENGINE{
    ...
    set_square(n_sq_n){
        ...
        var piece_id = this.piece(n_sq_n)
        var pr_rank  = piece_id < 5 ? 0: 1;
        var _2ndindex = (this.EB[n_sq_n] >> 4 & 0x7) // get the file of piece
        // console.log("board's square : ", this.EB[n_sq_n])
        var inde = (__BORANK[this.pl_type(n_sq_n)][pr_rank] << 4) + ( _2ndindex + 8) ;
        // console.log("2index : " + _2ndindex);
...
class Engine{
    ...
    move_encode(sq_n, n_sq_n){
        return (((sq_n & x88_MASK) << x88_BITS) | this.piece(sq_n, true)) | (((n_sq_n & x88_MASK) << x88_BITS) | this.piece(n_sq_n, true)) 
    }

    move_decode(c_sq_n){
        return [[(c_sq_n >> x88_MOVE_BITS + PIECE_BITS) & x88_MASK, (c_sq_n >> x88_MOVE_BITS) & PIECE_MASK], [((c_sq_n >> PIECE_BITS) & x88_MASK), c_sq_n & PIECE_MASK]]
    }
     ...
     * @param {int} n_sq_n 
     */
    move(sq_n, n_sq_n, valid_moves = null){
        n_sq_n = sq_n
        sq_n = n_sq_n ? n_sq_n : this.sq_n
        if(!this.sq_n || sq_n == n_sq_n) return

        if(valid_moves){
            valid_moves = valid_moves.map(a => a & x88_MASK)
            console.log("valid_moves", valid_moves)
            if(!valid_moves.includes(sq_n & x88_MASK)) return
        }
    ...
    undo(){        
        ...
        var index = (__BORANK[this.pl_type(p[0])][this.piece(p[0]) < 5 ? 0: 1] << 4) + (this.EB[p[0]] >> 4 & 0x7)  ;
        this.EB[index] = p[0];
        index = (__BORANK[this.pl_type(n[0])][this.piece(n[0]) < 5 ? 0: 1] << 4) + (this.EB[n[0]] >> 4 & 0x7)  ;
        this.EB[index] = n[0];
        
        this.EB[p[0]] = (p[0] & 3) | p[1]
        this.EB[n[0]] = (n[0] & 3) | n[1]
        ...
        for(var i = 0; i < 2; i++){
            ...
            for(var j = 0; j < 2; j++){
                ...
                for(var k = 0; k < 8; k++){
                    ...
                    var moves = this.valid_moves(piece_sq, null, turn)
                    // console.log("piece id ", this.piece(piece_sq, true), " t mov ", moves)
                    if(moves['score']){
                        // if(moves['score'] != [0, 0]) console.log("piece id ", this.piece(piece_sq, true), " scores ", moves['score'])
                        if(moves['score'].length > 0)
                            score +=  moves['score'].reduce((a, b) => a + b) // moves['moves'].length * offset_multiplier
                    }else continue
                        // score += moves['moves'].length * offset_multiplier
                        // score += moves['moves'].length * offset_multiplier + Math.sum(...moves['score'])
    ...
    next_move(sq_n = 0, n_sq_n = 0, depth = 5){
    ...
    get_all_moves(turn){
        ...
        for(var i = 0; i < 2; i++){
            ...
            for(var j = 8; j < 16; j++){
                ...
                var temp_moves = this.valid_moves(n_sq_n, turn) // ,  Dir offset gives all direction for a piece on square wrt current player king
     ...
     * @param {int} turn 
     */
    valid_moves(sq_n, directions = null, turn = this.turn, move = false){
        if(!this.piece_area(sq_n)) return [];
        // if(attacked) this.c_sq_n = sq_n
        var moves = [];
        var piece = this.piece(sq_n);
        ...
        if(move){ 
            this.valid_moves = moves['moves'].map(a => a & x88_MASK)
